...
from app.models import Team, TeamMember, User, Checkin, Blocker
from app.core.dependencies import get_current_user, require_manager, require_team_access, require_team_manager
from app.utils.plan_limits import require_starter as _require_starter_base, user_has_starter_access
...
def _require_starter(user: User) -> None:
    _require_starter_base(user, "Reports")
...
async def team_summary(
    ...
    return {
        "team": {"id": str(team.id), "name": team.name, "plan": manager.plan if manager else "free"},
        "period_days": period_days,
        "start_date": str(since),
        "end_date": str(today),
        "overview": {
